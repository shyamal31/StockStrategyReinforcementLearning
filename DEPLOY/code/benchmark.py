...
class Benchmark:
    ...
    def calculate_vwap(self, idx, shares):
        ...
        bid_prices = [self.data[f'bid_price_{i}'].iloc[0] for i in range(1,6)]
        bid_sizes = [self.data[f'bid_size_{i}'].iloc[0] for i in range(1,6)]
        cumsum = 0
        ...
        return np.sum(np.array(bid_prices)[:idx+1] * np.array(bid_sizes)[:idx+1]) / np.sum(bid_sizes[:idx+1])
    ...
    def compute_components(self, alpha, shares, idx):
        ...
        Slippage = (self.data['bid_price_1'].iloc[0] - actual_price) * shares  # Assumes bid_price is in your dataset
